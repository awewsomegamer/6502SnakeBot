...
mos6502::mos6502(void* u, BusRead r, BusWrite w, ClockCycle c)
   : reset_A(0x00)
   , reset_X(0x00)
   , reset_Y(0x00)
   , reset_sp(0xFD)
...
uint16_t mos6502::Addr_ABI()
{
   ...
   effH = Read(user, (abs & 0xFF00) + ((abs + 1) & 0x00FF) );
#else
   effH = Read(user, abs + 1);
#endif
...
{
   uint16_t addr = (Read(user, pc++) + X) & 0xFF;
   return addr;
}

uint16_t mos6502::Addr_ZEY()
{
   uint16_t addr = (Read(user, pc++) + Y) & 0xFF;
   return addr;
}
...
uint16_t mos6502::Addr_INX()
{
   ...
   zeroL = (Read(user, pc++) + X) & 0xFF;
   zeroH = (zeroL + 1) & 0xFF;
   addr = Read(user, zeroL) + (Read(user, zeroH) << 8);
...
uint16_t mos6502::Addr_INY()
{
   ...
   zeroL = Read(user, pc++);
   zeroH = (zeroL + 1) & 0xFF;
   addr = (baseL = /* ASSIGN */ Read(user, zeroL)) + (Read(user, zeroH) << 8) + Y;
   crossed = (baseL + Y) > 255;
...
void mos6502::Reset()
{
   ...
   uint8_t pcl = Read(user, rstVectorL);
   uint8_t pch = Read(user, rstVectorH);
   pc = (pch << 8) + pcl;
...
void mos6502::Svc_IRQ()
{
   ...
   uint8_t pcl = Read(user, irqVectorL);
   uint8_t pch = Read(user, irqVectorH);
   pc = (pch << 8) + pcl;
   return;
}
...
void mos6502::Svc_NMI()
{
   ...
   uint8_t pcl = Read(user, nmiVectorL);
   uint8_t pch = Read(user, nmiVectorH);
   pc = (pch << 8) + pcl;
   return;
}
...
void mos6502::Op_BRK(uint16_t src)
{
   ...
   SET_INTERRUPT(1);
   pc = (Read(user, irqVectorH) << 8) + Read(user, irqVectorL);
   return;
}
...
{
   unsigned int tmp = A - Read(user, src);
   SET_CARRY(tmp < 0x100);
   SET_NEGATIVE(tmp & 0x80);
   SET_ZERO(!(tmp & 0xFF));
   return;
...
{
   unsigned int tmp = X - Read(user, src);
   SET_CARRY(tmp < 0x100);
   SET_NEGATIVE(tmp & 0x80);
   SET_ZERO(!(tmp & 0xFF));
   return;
...
{
   unsigned int tmp = Y - Read(user, src);
   SET_CARRY(tmp < 0x100);
   SET_NEGATIVE(tmp & 0x80);
   SET_ZERO(!(tmp & 0xFF));
   return;
...
{
   Write(user, src, A & X); // most simple illegal here
}
